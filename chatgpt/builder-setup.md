...
For updating an already-published GPT, follow
[`publishing-knowledge.md`](publishing-knowledge.md) instead — it covers the
supported-capability limits and the smallest reliable publish procedure.
...
6. Upload all three files from `chatgpt/knowledge/` as Knowledge.
...
Instead of manual upload, build `dist/interview-journey-chatgpt.zip` with `./scripts/package-chatgpt-gpt.sh` (or `.ps1`) and use its contents (`instructions.md`, `knowledge/*.md`, `conversation-starters.md`) as the paste-ready source for the same steps above. The package also includes `deployment-release.json`, which records the repository commit and per-bundle content digests for the build.
